...
Оба скрипта **отказываются запускаться**, если в `zap-rules.tsv` не осталось ни одного FAIL-правила:
с флагом `-I` такой прогон не может упасть, что бы ни нашёл, и зелёный результат означал бы только сам
себя.

### 3.1. Авторизованные профили (clerk / chief / employee)

Анонимный baseline видит форму входа и почти ничего больше — реестр, карточка документа, архив и
обмен живут за сессией. Прогон под ролью (план СЭД, этап 11):

```bash
export ZAP_CLERK_USER=... ZAP_CLERK_PASS=...
./infra/security/zap-authenticated.sh https://staging.<домен> clerk
./infra/security/zap-authenticated.sh https://staging.<домен> chief      # ZAP_CHIEF_USER/PASS
./infra/security/zap-authenticated.sh https://staging.<домен> employee   # ZAP_EMPLOYEE_USER/PASS
```

Прогонять **каждую роль отдельно**: что скрипт сумеет обойти — ровно то, что разрешают права этой
роли, и разница между отчётами и есть смысл упражнения.

Что важно знать про эти прогоны:

- **Только стейдж.** Паук ходит настоящими запросами от настоящего пользователя.
- **Учётка должна быть без 2FA-гейта** — аккаунт, которого `TotpEnrollmentGuard` заворачивает на
  enrollment, просканировать нельзя (см. §4). Для clerk/chief это значит завести отдельные
  стендовые учётки с включённым TOTP или без прав из `PERMISSIONS_REQUIRING_2FA`.
- **Отчёт содержит cookie сессии** (ZAP сохраняет отправленные запросы). `zap-report-<role>.html` —
  это секрет; скрипт по завершении разлогинивает сессию (`ZAP_KEEP_SESSION=1` отключает).
- `/api/auth/logout` и смена пароля исключены из обхода: один запрос на logout убил бы сессию
  посреди прогона, и дальше всё выглядело бы «чисто».
