...
Сервис `backup` в compose.prod каждую ночь делает зашифрованный restic-бэкап: `pg_dump -Fc` БД + снимок
томов MinIO/GeoServer/УЦ/Caddy, с ротацией 30 дней + 12 месячных. Перед первым запуском задайте
`RESTIC_PASSWORD` в `.env` и **сохраните его копию оффлайн** (без него бэкапы невосстановимы). Настройте
хранение репозитория вне сервера и проверяйте восстановление restore-drill'ом ежеквартально — полная
процедура (эксплуатация, off-site, drill, аварийное восстановление) в **docs/runbook-backup.md**. Бэкап без
проверенного восстановления бэкапом не считается.
