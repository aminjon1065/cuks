-- Incident vector tiles for the operational map (docs/modules/10 §4).
--
-- Filters are parsed once and fail closed: a non-empty malformed value produces
-- an empty tile instead of silently widening the result set. At low zooms the
-- function clusters incidents on a global WebMercator grid. The grid is stable
-- across adjacent tiles and the query envelope is expanded by one cell so a
-- cluster at a tile edge is calculated from the complete bucket. At z >= 11 the
-- source emits individual marker points.
...
DECLARE
  ...
  valid_filters boolean :=
    ...
    AND (raw_from IS NULL OR f_from IS NOT NULL)
    AND (raw_to IS NULL OR f_to IS NOT NULL)
...
BEGIN
  ...
  WITH filtered AS (
    ...
    SELECT
      ...
      f.cluster_count
    FROM features f
    -- Assign a marker/cluster to exactly one tile. The expanded query envelope
    -- is only for complete edge-bucket aggregation, not duplicate rendering.
    WHERE ST_Intersects(f.feature_geom, env)
  )
