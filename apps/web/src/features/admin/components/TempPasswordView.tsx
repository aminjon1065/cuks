  ...
  return (
    <div>
      ...
      <div className="mt-1 flex items-center gap-2">
        <code className="flex-1 select-all rounded-sm border border-border bg-surface-2 px-3 py-2 font-mono text-[13px] text-text">
          {value}
        </code>
        <button
          type="button"
...
export function TempPasswordView({ data }: { data: TempPasswordDto }): React.JSX.Element {
  ...
  return (
    <div className="space-y-3">
      ...
      <CopyRow label={t('users.tempPassword.username')} value={data.username} />
      <CopyRow label={t('users.tempPassword.password')} value={data.tempPassword} />
    </div>
  );
}
