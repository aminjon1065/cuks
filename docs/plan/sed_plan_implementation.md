...
- [ ] Предустановленные типы: приказ, протокол, служебная записка — только после
  подтверждения справочника заказчиком.
- [ ] Templates + routes по типу.
- [ ] Targets users/positions/org units.
- [ ] Distribution batch переиспользует acquaintances.
- [ ] Приказы требуют signature/register в policy, если так задано type config.
- [ ] Из документа создаётся связанная task/resolution, а не дублируется task logic.
- [ ] Статусы ознакомления и исполнения видны в карточке.
- [ ] Отчёт по ознакомлению экспортируется с visibility policy.
