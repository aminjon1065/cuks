...
- **Фаза**: 3 (Документооборот) — начата; задача 3.1 готова
- **Последняя сессия**: 2026-07-15 — задача 3.1 (справочники ДОУ: журналы+нумерация, корреспонденты, номенклатура)
- **Ветка**: main
...
- **Живая проверка НЕ прогнана**: dev-стек не поднялся — **Docker Desktop в этой неинтерактивной
  сессии не стартовал** (WSL-бэкенд/GUI-логин недоступны). Миграция сгенерирована drizzle-kit и
  просмотрена глазами; e2e Playwright (`docflow-settings.spec.ts` — создание/удаление журнала) и живой
  тест конкурентной нумерации (50 параллельных регистраций) **написаны, требуют прогона на стеке**.
  Команды для догона: `docker compose … up -d` → `db:migrate && db:seed` →
  `pnpm --filter @cuks/web exec playwright test docflow-settings`.
...
- **Живой прогон отложен** (Docker в сессии недоступен): `migrate`+`seed`, e2e Playwright и тест
  конкурентной нумерации 50× — написаны, требуют стека. Приёмка «нумерация без дыр/дублей при 50
  параллельных регистрациях» — критерий 3.12.
