...
- [~] Составить data inventory текущей production/dev БД перед backfill. — **снят 2026-08-02**,
  обоснование ниже («Снятые пункты»). Коротко: окно закрылось (все схемные этапы 2–10 и миграции
  0047–0060 применены), а то, ради чего инвентарь был нужен, закрыто по факту —
  `infra/scripts/restore-verify.sh` и §1 чеклиста миграции. Остаточный риск назван явно.
...
Снятые пункты (2026-08-02):

**«Составить data inventory текущей production/dev БД перед backfill» — снят.**

*Почему окно закрылось.* Пункт был явно перенесён «в первый этап со схемными изменениями (2)».
Этапы 2–10 отработаны, миграции 0047–0060 применены, включая все три записи данных трека.
Инвентарь «перед backfill» после backfill составить нельзя — это уже не тот артефакт.

*Почему это не оставляет дыру.* Инвентарь нужен был как страховка от записи данных по неверным
предположениям о том, что в базе лежит. Фактические записи данных трека таких предположений не
делают:

- `0055_hot_the_watchers` (единственная data-only миграция) выводит `archived_at` из
  `updated_at`/`created_at` самой строки, а срок — из дела ровно так же, как это делает
  `planRetention` в рантайме. Там, где данных нет, она их не выдумывает: `archived_by` остаётся
  `NULL`, потому что кто подшил документ до этого релиза — неизвестно;
- `backfill:docflow-files` имеет `--dry-run`, повторный запуск — no-op (трогает только узлы,
  которые ещё вне системного дерева), коммитит по одному документу, так что прерванный прогон
  возобновляется простым перезапуском.

*Чем закрыто по факту.* Проверки, ради которых инвентарь и понадобился бы, теперь выполняются
после применения и автоматически: `infra/scripts/restore-verify.sh` (таблицы на месте,
`document_files` разрешается, у версий файлов есть объекты в бакете, счётчики регистрации не
отстали от выданных номеров), плюс §1 и §4 `docs/migration-rollback-checklist.md`, где
**проверенный** бэкап — предусловие миграции на живой установке, а не пожелание.

*Остаточный риск — честно.* Пост-фактум проверка слабее предполётного пересчёта ровно в одном:
она не ловит **тихое частичное применение**. Если у заказчика есть установка с данными ДОУ
старше трека, «до» никто не считал, и сверить с ним нечего — расхождение будет видно только как
нарушенный инвариант, а не как недостача N строк. Смягчение: `0055` идемпотентна и
перезапускаема, а `restore-verify.sh` прогоняется до того, как пользователей пускают внутрь.
Если заказчик назовёт такую установку, правильным действием будет не воскрешать этот пункт, а
снять с неё дамп и прогнать проверку на копии — это то же знание, полученное дешевле и позже.
...
- [x] метрики длительности/ошибок. — worker пишет в Redis запись о каждом проходе трёх
  регламентных развёрток (`deadlines`, `retention`, `audit-maintenance`): длительность, исход и
  счётчики того, что проход реально сделал. Контракт — `packages/shared/src/metrics/index.ts`
  (ключ, TTL, кодирование, пороги «протухания» по каждой задаче); api отдаёт их в
  `GET /admin/health`, экран «Здоровье» показывает панель «Регламентные развёртки» с четырьмя
  состояниями (норма / упала / давно не выполнялась / нет данных). Учёт не может испортить
  измеряемую работу: упавший проход бросает прежнюю ошибку дальше (BullMQ видит failed и
  ретраит, как раньше), а недоступный Redis не проваливает проход, который своё сделал.
