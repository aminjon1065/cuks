...
- **Фаза**: 3 (Документооборот) — задача 3.1 готова; 3.2 — **бэкенд готов** (веб-UI + adversarial-review остаются)
- **Последняя сессия**: 2026-07-15 — задача 3.2 (documents/document_files, статусная машина) — бэкенд
- **Ветка**: main
...
| 3 Документооборот | 🟡 задача 3.1 готова          | —                  |
| 4 Задачи          | ⬜                           | —                  |
| 5 Чат             | ⬜                           | —                  |
| 6 Звонки          | ⬜                           | —                  |
| 7 Hardening       | ⬜                           | —                  |
...
### 2026-07-15 — Фаза 3: задача 3.2 (documents/document_files, статусная машина) — БЭКЕНД (в работе)

**Сделано** (по `docs/modules/11` §3/§4; коммиты `11fbe55`, `a79a533`, `9648f45`):

- **`feat(shared)`**: enum жизненного цикла документа (`draft → on_route → pending_registration →
  registered → in_progress → completed → archived` + `rejected`/`recalled`), политика переходов
...
- **`feat(db)`**: миграция `0028` — `app.documents` (журнал/рег-номер null до регистрации, статус,
  ...
  вложения — отдельными строками). Применена на dev-БД.
- **`feat(docflow)`**: `DocumentsService`/`DocumentsController` — создание/список(очереди
  ...
  **Регистрация** (`POST /documents/:id/actions/register`) в одной транзакции назначает журнал и мнёт
  ...
  видят не-ДСП реестр; **ДСП — только допуск-список**, вне scope — 404 (существование не утекает).

**Тесты/проверка**:

- Корневой gate typecheck/lint/test/build — зелёный: **API 250** (+4 статус-машина), **shared 59**
  (+6 DTO/переходы), web/worker/ui/db без изменений. Новые unit: `planDocumentStatusChange`, document-DTO.
- **Живая проверка** (реальный API + Postgres 17, e2e Playwright `docflow-documents.spec.ts`, зелёный):
  create → **register** (номер `П-2026/NNNN` через счётчик 3.1) → статус (валидный переход + отказ на
  недопустимый `in_progress→draft` = 422); **ДСП-документ**: автор видит, сторонний сотрудник — 404.

**Остаётся по 3.2**: веб-UI (список «Мои документы»/очереди + карточка документа со степпером статуса и
файлами), adversarial-review диффа, затем закрытие задачи (тик ROADMAP 3.2).

**Инфра-замечание**: `docker compose up -d` в этой сессии пересоздал контейнер redis по текущему compose
(`6379:6379`), а `REDIS_URL` в `.env` = `6380` — рассинхрон compose/`.env`. Из-за этого логин (сессии в
Redis) падал, пока e2e не запущен с `REDIS_URL=redis://localhost:6379`. Нужно свести порт (compose ↔ .env)
к одному значению — не относится к коду 3.2.
