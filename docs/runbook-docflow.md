...
что и api. Своего HTTP-эндпоинта здоровья у него нет: «жив ли он» видно на `/app/admin/health` — по
панели «Регламентные развёртки» и очередям (с них и начинается «Диагностика» ниже), а дальше — по
логам и `dc ps`.
...
| `audit-maintenance` | `ensure` при старте worker'а и repeatable `ensure`, `0 3 1 * *`: партиции `audit.audit_log` на два месяца вперёд |
...
**Начинать с экрана `/app/admin/health` → «Регламентные развёртки»** (право `admin.system.monitor`,
он же `GET /api/v1/admin/health`). Три карточки — «Сроки документов» (`deadlines`), «Сроки хранения»
(`retention`), «Обслуживание журнала аудита» (`audit-maintenance`) — показывают последний прогон
каждого задания: когда закончился, сколько шёл, что насчитал и не упал ли; при сбое текст ошибки
стоит прямо в карточке. До неё «прошёл ли сегодняшний проход» можно было узнать только грепом лога —
а проход, которого не было, в логе выглядит ровно так же, как проход, до которого ещё не дошли руки.
Контракт и пороги — `docs/modules/16-admin.md` §7.

| Состояние | Что это для СЭД |
|---|---|
| `ok` («Норма») | проход **дошёл до конца** не позже своего порога. Нули в счётчиках при этом законны — «нечего было делать» и «не работает» различаются по состоянию, а не по числам. Но «Норма» — это не «всё удалось»: `sweep()` и у `deadlines`, и у `retention` ловит исключение по отдельному элементу, пишет его в лог и продолжает (`deadline item failed`, `task deadline item failed`, `route deadline item failed`, `failed to purge trashed node, skipping`), так что проход, где провалилась половина резолюций, всё равно зелёный и `ok: true`. Частные отказы есть только в логе: `dc logs worker --since 24h \| grep -E "item failed\|skipping"` |
| `failed` («Сбой») | процессор бросил исключение, текст — прямо на экране. Смотреть лог worker'а по этому заданию |
| `stale` («Давно не выполнялась») | проходов больше нет. Для `deadlines` это значит, что **напоминания и эскалации не производятся вовсе** — ни по резолюциям, ни по шагам маршрутов, ни по задачам. Из реестра это не видно никак: документы на месте, сроки в карточках нарисованы, просрочка считается на лету при отображении — просто никого о ней не уведомляют. Пропущенные слоты не догоняются (§2), поэтому каждый день простоя — это день без напоминаний, который сам не наверстается. Для `retention` — никто не помечает архивных кандидатов (§6); для `audit-maintenance` — не нарезаются партиции аудита вперёд |
| `unknown` («Нет данных», в карточке — «Ещё не запускалась») | записи о прогоне нет. Штатно — до первого прогона после установки или очистки Redis: у `deadlines`/`retention` это до суток (ждём ночной слот), у `audit-maintenance` — до ближайшего старта worker'а, а не до месяца (`ensure` ставится и при старте процесса, и по расписанию). Там же, при старте, расписания и регистрируются, поэтому после очистки Redis worker нужно перезапустить. Если состояние держится дольше — задание не идёт, разбирать как `stale` |

Пороги устаревания разные по заданиям: суточным `deadlines` и `retention` — 26 ч, месячному
`audit-maintenance` (`0 3 1 * *`) — 33 суток, иначе он висел бы в «Давно не выполнялась» весь месяц
между прогонами.
`failed` или `stale` у любого из трёх переводят общий статус экрана в «Есть проблемы», поэтому
вставший ночной проход виден уже с шапки, до раскрытия панели.

Счётчики прохода — в его собственных единицах: у `deadlines` это `scanned` (сколько контрольных
резолюций просмотрено), `emitted` (сколько строк outbox по резолюциям создано), `taskEmitted` и
`routeEmitted`. У упавшего прохода счётчиков нет вовсе — он прервался на середине.

Само по себе `ok` у `deadlines` **не** доказывает, что уведомления дошли: worker только пишет строки
в `app.notification_outbox`, рассылает их диспетчер в api. «Проход `ok`, а писем нет» — это §2,
вторая неисправность, а не эта.

Записи о прогонах живут в Redis (`metrics:jobrun:<задание>`, TTL 45 суток — запись обязана пережить
самый длинный порог устаревания, 33 суток у `audit-maintenance`, иначе месячное задание уходило бы
из «Нормы» сразу в «Нет данных», минуя «Давно не выполнялась») и намеренно не могут испортить
измеряемую работу: запись не бросает исключений. Недоступный на чтение Redis даёт `unknown` сразу у
всех трёх заданий; не долетевшая запись оставляет прошлый прогон, и тот сперва становится `stale` по
своему порогу, а исчезает лишь много позже. `failed` из этого не выходит никогда — панель не объявит
упавшим проход, который на самом деле отработал. И наоборот: сам учёт не меняет поведения BullMQ —
упавший проход по-прежнему переспрашивается и виден в счётчике `failed` виджета очередей.

Дальше — лог и процесс:
...
| Развёртка `deadlines` («Сроки документов», §7) | «Норма», прогон не старше 26 ч | «Давно не выполнялась» (напоминаний и эскалаций нет вовсе), «Сбой», «Нет данных» дольше суток |
| Развёртка `retention` («Сроки хранения») | «Норма», прогон не старше 26 ч | «Давно не выполнялась» / «Сбой» |
| Развёртка `audit-maintenance` («Обслуживание журнала аудита») | «Норма» (порог 33 суток — задание месячное) | «Давно не выполнялась» / «Сбой» |
| Длительность последнего прогона (в карточке развёртки) | секунды, реже минуты | прогон, ушедший далеко за минуты, и тем более сопоставимый с интервалом собственного расписания (проходы начнут накладываться). Сравнить с «обычной» длительностью панель не даёт: она хранит только последний прогон (§9), а в лог длительность не пишется вовсе — тревога только по абсолютной величине |
...
- У процесса worker нет эндпоинта здоровья: его состояние видно по регламентным развёрткам и
  очередям (`/app/admin/health`), логам и `dc ps`.
- Панель «Регламентные развёртки» хранит только **последний** прогон каждого задания, без истории:
  «сколько шёл вчера» по ней не сравнить, и алертов она не шлёт — это по-прежнему дашборд
  (`docs/runbook-monitoring.md`). Молчание месячного `audit-maintenance` замечается не раньше
  чем через 33 суток — таков его порог.
