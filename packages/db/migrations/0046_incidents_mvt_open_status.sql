-- Duty-mode map filter: status=open means all non-closed incidents (same
-- semantics as the dashboard «active» list). Single status values keep working.
...
DECLARE
  ...
  valid_filters boolean :=
    (raw_status IS NULL OR raw_status IN ('reported', 'active', 'localized', 'eliminated', 'closed', 'open'))
...
BEGIN
  ...
  WITH filtered AS (
    ...
    WHERE i.deleted_at IS NULL
      ...
      AND (
        raw_status IS NULL
        OR (raw_status = 'open' AND i.status <> 'closed')
        OR (raw_status <> 'open' AND i.status = raw_status)
      )
